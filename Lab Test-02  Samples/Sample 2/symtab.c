...
int type_check(int type1, int type2)
{
    ...
    else if (type1==REAL_TYPE && type2==CHAR_TYPE)
    {
        printf("Type REAL and Type CHAR are incompatiable\n");
        return (-1);    
    }
...
int type_check_2(int type1, int type2)
{
    if (type1==INT_TYPE && type2==INT_TYPE)
    {
        printf("int number is assigned to int\n");
        return (INT_TYPE);
    }
    else if (type1==REAL_TYPE && type2==REAL_TYPE)
    {
        printf("float number is assigned to float\n");
        return (INT_TYPE);
    }
    else if (type1==CHAR_TYPE && type2==CHAR_TYPE)
    {
        printf("char is assigned to char\n");
        return (INT_TYPE);
    }
    else if (type1==INT_TYPE && type2==REAL_TYPE)
    {
        printf("float number is used instead of integer\n");
        return (-1);
    }
    else if (type1==INT_TYPE && type2==CHAR_TYPE)
    {
